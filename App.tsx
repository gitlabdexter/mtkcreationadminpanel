...
const App: React.FC = () => {
  ...
  const [isAuthenticated, setIsAuthenticated] = useLocalStorage<boolean>('dx-vpn-admin-auth', false);
  ...
  const toggleView = () => {
    ...
    // If switching to admin view, reset authentication state for a fresh login
    if (view === 'user') {
  ...
  return (
    <div className={`min-h-screen font-sans text-black dark:text-white bg-dx-light dark:bg-dx-dark transition-colors duration-300`}>
      ...
      <main>
        {view === 'admin' ? 
          (isAuthenticated ? <AdminPanel setIsAuthenticated={setIsAuthenticated} /> : <LoginPage setIsAuthenticated={setIsAuthenticated} />) 
          : <UserApp />}
      </main>
    </div>
  );
